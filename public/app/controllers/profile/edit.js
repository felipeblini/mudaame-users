angular.module('myApp').controller('EditProfileController', 
  ['$scope', '$routeParams', '$location', '$translate', '$timeout', 'UserErrorService', 'NavigationService', 'EntityUtilService', 'SecurityService', 'ProfileService', 
  function($scope, $routeParams, $location, $translate, $timeout, UserErrorService, NavigationService, EntityUtilService, SecurityService, ProfileService) {
	...
	$scope.add = function () {
		...
		ProfileService.add(dataToServer($scope.obj)).then(function (httpResponse) {
			if($scope.parent) {
	...
	$scope.update = function () {
		$scope.uiWorking = true;
		ProfileService.update(dataToServer($scope.obj)).then(function () { //httpResponse
				returnBack();
	...
	$scope.delete = function () {
		$scope.uiWorking = true;
		ProfileService.delete($scope.obj._id).then(returnBack, errorHandlerDelete, progressNotify);
	};
	...
	$scope.gotoEdit = function() {
		$location.path('/profile/edit/' + $routeParams.id);		
	};

	$scope.gotoDelete = function() {
		$location.path('/profile/delete/' + $routeParams.id);		
	};
	...
	function init() {
		...
		if ($routeParams.id) {
			ProfileService.getDocument($routeParams.id).then(loadData, errorHandlerLoad);		
		}
